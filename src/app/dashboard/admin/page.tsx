...
export default function AdminDashboard() {
  ...
  const analyticsData = useMemo(() => {
    ...
    return {
      screening: {
        summary: [
          { name: 'Eligible', value: eligible.length, fill: '#10B981', insight: "Positive qualification trend." },
          { name: 'Ineligible', value: ineligible.length, fill: '#EF4444', insight: "Analysis required for denials." }
        ],
        reasons: reasonsData,
  ...
  const handleMarkAllRead = () => {
    ...
    unread.forEach(n => updateDocumentNonBlocking(doc(db, "notifications", n.id), { status: "read" }));
    toast({ title: "Audit Synchronized", description: `All ${unread.length} logs marked as read.` });
  };
    ...
    <DashboardLayout role="admin">
      <div className="max-w-7xl mx-auto space-y-12 pb-20">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground font-medium uppercase text-[10px] tracking-widest mt-1">Operational Analysis & Platform Audit</p>
          </div>
        </div>
                  ...
                  {!analyticsData || !mounted ? <div className="py-20"><Loader2 className="animate-spin h-10 w-10 text-primary/20" /></div> : (
                    <>
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                          <PieChart>
                            <Pie
                              ...
                              dataKey="value"
                              stroke="none"
                              onClick={(data) => { setSelectedInsight(data); setIsInsightOpen(true); }}
                            >
                      ...
                      <div className="grid grid-cols-2 gap-4 w-full mt-4">
                        <div className="text-center p-4 bg-emerald-50 rounded-2xl border border-emerald-100 shadow-sm">
                        ...
                        <div className="text-center p-4 bg-rose-50 rounded-2xl border border-rose-100 shadow-sm">
                    ...
                    <div className="grid md:grid-cols-2 gap-12">
                      <div className="space-y-8">
                        <div className="space-y-2">
                          ...
                          <Badge className="text-xs font-black uppercase px-4 py-1.5 border-none shadow-sm rounded-xl block w-fit">{viewingAuditLog.type}</Badge>
                        </div>
                        <div className="space-y-2">
                          <Label className="text-[10px] font-black uppercase text-primary/40 tracking-widest">Objective Description</Label>
                        ...
                        <div className="space-y-2">
                          ...
                          <p className="font-bold text-secondary flex items-center gap-2"><Clock className="h-4 w-4" />{viewingAuditLog.createdAt ? format(new Date(viewingAuditLog.createdAt), "PPPP HH:mm:ss") : '---'}</p>
                        </div>
                      </div>
                      <div className="space-y-8">
                        <div className="space-y-2">
                          ...
                          <div className="flex items-center gap-3 bg-muted/30 p-4 rounded-2xl border">
                            ...
                            <div className="space-y-0.5">
                              ...
                              <p className="text-[10px] text-muted-foreground font-medium">Actor ID: {viewingAuditLog.targetUserId || 'System/Guest'}</p>
                            </div>
                          </div>
                        </div>
                        <div className="space-y-2">
                          ...
                          <div className="p-4 bg-primary/5 rounded-2xl border-2 border-dashed border-primary/10 flex items-center justify-between">
                            <p className="text-sm font-black text-primary">{viewingAuditLog.referenceCode || viewingAuditLog.referenceId || 'N/A'}</p>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="pt-10 border-t flex justify-end">
                      <Button onClick={() => setViewingAuditLog(null)} className="rounded-2xl h-14 bg-primary text-white font-black px-12 shadow-xl hover:scale-105 transition-transform"><ArrowLeft className="mr-2 h-5 w-5" /> Back to Logs</Button>
                    </div>
                  </CardContent>
                </div>
              ) : (
                  ...
                  <CardHeader className="bg-primary p-8 text-white flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <CardDescription className="text-white/60 font-bold uppercase tracking-widest text-[10px] mt-1">Platform Activity Registry</CardDescription>
                      </div>
                    </div>
                    <div className="flex items-center gap-4">
                      <div className="flex items-center gap-3">
                        ...
                        <Button variant="ghost" size="sm" onClick={handleMarkAllRead} className="text-white/60 hover:text-white hover:bg-white/10 font-bold text-[10px] uppercase tracking-widest h-9 px-3 rounded-xl border border-white/10" disabled={!notifications?.some(n => n.status === 'unread')}><CheckCheck className="h-3 w-3 mr-1.5" />Mark all as read</Button>
                      </div>
                      <Select value={notifFilter} onValueChange={setNotifFilter}>
                        <SelectTrigger className="h-11 w-[200px] bg-white/10 border-none text-white font-bold rounded-xl focus:ring-0"><div className="flex items-center gap-2"><Filter className="h-4 w-4" /><SelectValue placeholder="All Activities" /></div></SelectTrigger>
                    ...
                    <div className="divide-y divide-primary/5">
                      {filteredNotifs.length > 0 ? (
                        filteredNotifs.map((notif) => (
                          <div key={notif.id} className={cn("p-8 transition-all cursor-pointer hover:bg-primary/[0.02] relative group flex flex-col md:flex-row md:items-center justify-between gap-6", notif.status === 'unread' && "bg-amber-50/30")} onClick={() => handleNotifClick(notif)}>
                            {notif.status === 'unread' && <div className="absolute left-0 top-0 bottom-0 w-1.5 bg-amber-500" />}
                            <div className="flex gap-6 items-start flex-1">
                              <Badge className="text-[10px] font-black uppercase px-4 py-1.5 border-none shadow-sm rounded-xl shrink-0 h-fit">{notif.type}</Badge>
                            ...
                            <div className="flex items-center gap-2 text-secondary font-black uppercase text-[10px] tracking-widest shrink-0">View Detail<ArrowRight className="h-4 w-4" /></div>
                          </div>
                        ))
                      ) : (
                        <div className="py-32 text-center space-y-4"><Inbox className="h-16 w-16 text-primary/5 mx-auto" /><p className="text-xs font-black uppercase text-muted-foreground tracking-widest">No activities recorded</p></div>
                      )}
                    </div>
                  </CardContent>
                </>
      ...
      <Dialog open={isInsightOpen} onOpenChange={setIsInsightOpen}>
        <DialogContent className="rounded-[3rem] max-w-4xl p-0 overflow-hidden border-none shadow-2xl flex flex-col max-h-[90vh]">
          <DialogHeader className={cn("p-10 text-white flex flex-row items-center justify-between", selectedInsight?.name === 'Eligible' ? "bg-emerald-600" : "bg-rose-600")}>
            <div className="space-y-1"><DialogTitle className="text-4xl font-black tracking-tighter">{selectedInsight?.name} Segment Analysis</DialogTitle><DialogDescription className="text-white/60 font-black uppercase tracking-[0.2em] text-xs">Factors & Statistics</DialogDescription></div>
            <div className="p-4 bg-white/20 rounded-[2rem]">{selectedInsight?.name === 'Eligible' ? <ShieldCheck className="h-10 w-10" /> : <XCircle className="h-10 w-10" />}</div>
          </DialogHeader>
          <div className="p-10 space-y-10 flex-1 overflow-y-auto">
            <Card className="border-none bg-muted/30 rounded-[2rem] p-8 text-center"><p className="text-[10px] font-black uppercase text-muted-foreground tracking-widest mb-1">Volume</p><p className="text-4xl font-black text-primary">{selectedInsight?.value} Applicants</p></Card>
            <div className="p-8 bg-primary/5 rounded-[2.5rem] border-2 border-dashed border-primary/10 space-y-4"><div className="flex items-center gap-3"><Lightbulb className="h-6 w-6 text-primary" /><h4 className="text-sm font-black uppercase tracking-widest text-primary">Recommendations</h4></div><p className="text-xs text-muted-foreground font-bold leading-relaxed">{selectedInsight?.insight}</p></div>
          </div>
          <DialogFooter className="p-8 bg-muted/30"><Button onClick={() => setIsInsightOpen(false)} className="w-full h-14 bg-primary text-white font-black rounded-2xl shadow-xl">Close Analysis</Button></DialogFooter>
        </DialogContent>
      </Dialog>
    </DashboardLayout>
  );
