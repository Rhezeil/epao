...
export default function AdminUsersPage() {
  ...
  const handleAddClient = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: "Registration Complete", description: `${newClient.name} added. Password is password123.` });
  ...
  const handleDeleteUser = (userId: string) => {
    if (!db) return;
    updateDocumentNonBlocking(doc(db, "users", userId), { status: "Inactive" });
                  ...
                  {filteredUsers.map((client) => {
                    ...
                    return (
                      <TableRow key={client.id} className="hover:bg-primary/5 transition-colors group">
                        ...
                        <TableCell className="text-[10px] font-bold text-muted-foreground">{client.createdAt ? format(new Date(client.createdAt), "MMM dd, yyyy") : '---'}</TableCell>
                        <TableCell className="text-right px-8">
                          <div className="flex justify-end items-center gap-2">
                            {visitCount > 0 && <Badge variant="secondary" className="text-[9px] font-black h-6 rounded-lg px-2 bg-secondary/10 text-secondary">{visitCount} VISITS</Badge>}
                            <Button variant="ghost" size="icon" className="h-9 w-9 rounded-xl text-primary hover:bg-primary/5" onClick={() => { setSelectedClientId(client.id); setIsDetailsOpen(true); }}><Eye className="h-4 w-4" /></Button>
                            <DropdownMenu>
                              <DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-9 w-9 rounded-xl hover:bg-primary/5"><MoreVertical className="h-4 w-4" /></Button></DropdownMenuTrigger>
                              <DropdownMenuContent align="end" className="rounded-2xl w-56 p-2">
                                ...
                                <DropdownMenuSeparator />
                                <DropdownMenuItem onClick={() => handleDeleteUser(client.id)} className="text-destructive font-bold rounded-xl"><Trash2 className="mr-2 h-4 w-4" /> Deactivate Account</DropdownMenuItem>
                              </DropdownMenuContent>
                            </DropdownMenu>
                          </div>
                        </TableCell>
          ...
          <DialogContent className="rounded-[3rem] max-w-2xl p-0 overflow-hidden border-none shadow-2xl">
            ...
            <div className="p-10 space-y-8 max-h-[70vh] overflow-y-auto">
              <div className="p-4 bg-amber-50 rounded-2xl border border-amber-100 flex items-center gap-4">
                <div className="p-2 bg-amber-100 rounded-xl text-amber-700"><Lock className="h-5 w-5" /></div>
                <div>
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2"><Label className="text-[10px] font-black uppercase text-primary/40 ml-1">Assign Handling Attorney</Label>
                      <Select value={newClient.lawyerId} onValueChange={v => setNewClient({...newClient, lawyerId: v})}>
                        ...
                        <SelectContent>{lawyers?.map(l => <SelectItem key={l.id} value={l.id} className="font-bold">{l.firstName ? `Atty. ${l.firstName} ${l.lastName}` : l.email.split('@')[0]}</SelectItem>)}</SelectContent>
                      </Select>
                    </div>
                  </div>
                </div>
            ...
            <DialogFooter className="p-8 bg-muted/30"><Button variant="outline" onClick={() => setIsAddDialogOpen(false)} className="rounded-xl h-14 px-8 font-bold">Cancel</Button><Button onClick={handleAddClient} disabled={isSubmitting || !newClient.name || !newClient.mobile} className="rounded-xl h-14 bg-primary font-black px-12 shadow-xl">{isSubmitting ? <Loader2 className="animate-spin h-5 w-5 mr-2" /> : <ShieldCheck className="mr-2 h-5 w-5" />}Complete Registration</Button></DialogFooter>
          </DialogContent>
        </Dialog>
            ...
            <div className="p-10 overflow-y-auto flex-1">
              <Tabs defaultValue="profile" className="w-full">
                ...
                <TabsContent value="case" className="space-y-8 pt-6">
                  {activeCase ? <div className="space-y-6">
